  ...
  if(is.null(ps)){
    ...
    if(is.null(K)){
      ...
      if(is.null(threshold)){
        ...
        #initiliaze ig prior parameters based off of number of descendants for each node
        #gam_shape_p[p] has the prior shape parameter for node p
        #gam_rate_p[p] has the prior rate paraemter for node p
        gam_shape_p = rep(0,p)
        gam_rate_p = rep(0,p)
        for(a in 1:p){
        ...
        for(k in 1:K){
          for(a in 1:p){
            Sigma_ppk[a,a,k] = 1/stats::rgamma(1,shape = gam_shape_p[a],rate = gam_rate_p[a])
          }
        }
